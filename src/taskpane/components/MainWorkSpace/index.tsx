// src/taskpane/components/MainWorkSpace/index.tsx - UPDATED VERSION
...
import { useStyleConversion } from "../../hooks/useStyleConversion";
...
export const MainWorkspace: React.FC<MainWorkspaceProps> = ({ user }) => {
  ...
  // ✅ NEW: Style conversion hook
  const { convertStyle, isConverting } = useStyleConversion(
    selectedStyle,
    citedSourceIds,
    setCitedSourceIds,
    setSelectedStyle,
  ...
  // ✅ NEW: Handler for viewing source in document
  const handleViewInDocument = async (sourceId: string) => {
    // Implement scroll to first citation of this source
    console.log(`Viewing source ${sourceId} in document`);
  ...
  return (
    <div className={styles.container}>
      ...
      {/* ✅ NEW: Style Converter Component */}
      <StyleConverter
        currentStyle={selectedStyle}
        citedCount={citedSourceIds.size}
        onConvert={convertStyle}
      />
      ...
      <div className={styles.tabsContainer}>
        ...
        >
          ...
          {/* ✅ NEW: Used Sources Tab */}
          <Tab value="used">
            ✅ منابع استفاده شده
            {citedSourceIds.size > 0 && (
              <Badge appearance="filled" color="brand" className={styles.notesBadge}>
                {citedSourceIds.size}
              </Badge>
            )}
          </Tab>
